...
Item
{
    ...
    Column {
        LabelWithClickableImage {
            ...
            prDefaultImage: (idW3WSetup.prCurrentExpandedIndex === itemId)
            ...
            prPressedImage: (idW3WSetup.prCurrentExpandedIndex === itemId)
            ...
            onReleased:
            {
                cppW3WApplication.qmlLog("W3WSetupLabelWithDropDownDelegate.qml onReleased - index: " + index)
                toggleExpandState()
            }

            function toggleExpandState() {

                cppW3WApplication.qmlLog("W3WSetupLabelWithDropDownDelegate.qml toggleExpandState - index: "+ itemId)

                //Dropdown collapsed state
                if (idW3WSetup.prCurrentExpandedIndex === itemId)
                {
                    idW3WSetup.prCurrentExpandedIndex = -1
                    cppW3WAdaptor.w3wSetupListModel.collapseSetupDropDown(itemId)
                }
                //Dropdown expand state
                else
                {
                    idW3WSetup.prCurrentExpandedIndex = itemId
                    cppW3WAdaptor.w3wSetupListModel.expandSetupDropDown(itemId)
