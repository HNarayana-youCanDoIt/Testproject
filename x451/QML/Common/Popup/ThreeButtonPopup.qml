...
BasePopup {
    ...
    contentItem: Item {
        ...
        MouseArea{
            anchors.fill: parent
            onPressed: {
                cppUIBase.qmlLog("ThreeButtonPopup.qml: Popup Content Item Touched")
        ...
        GenericButton {
            ...
            onPressed: {
                cppUIBase.qmlLog("ThreeButtonPopup.qml, idFirstBtn: onPressed event received")
                // Deactivate VR session
                cppVRAdaptor.invDeactivateVRSession(HardkeyEnum.HKEY_STATE_RELEASED)
                // Deactivate phone bt vr
                cppPhoneAdaptor.invReqPhoneBtVrDeactivation()
        ...
        GenericButton {
            ...
            onPressed: {
                cppUIBase.qmlLog("ThreeButtonPopup.qml, idSecondBtn: onPressed event received")
                // Deactivate VR session
                cppVRAdaptor.invDeactivateVRSession(HardkeyEnum.HKEY_STATE_RELEASED)
                // Deactivate phone bt vr
                cppPhoneAdaptor.invReqPhoneBtVrDeactivation()
        ...
        GenericButton {
            ...
            onPressed: {
                cppUIBase.qmlLog("ThreeButtonPopup.qml, idThirdBtn: onPressed event received")
                // Deactivate VR session
                cppVRAdaptor.invDeactivateVRSession(HardkeyEnum.HKEY_STATE_RELEASED)
                // Deactivate phone bt vr
                cppPhoneAdaptor.invReqPhoneBtVrDeactivation()
        ...
        Connections{
            ...
            onSigRotaryUpdatePopup:{
                cppUIBase.qmlLog("ThreeButtonPopup.qml: sigRotaryUpdate: iKeyState: "
                                 + iKeyState + " iKeyCode : " + iKey)

                PopupManager.invResetPopupTimer()
                var prPreviousRREIndex = prCurrentRREIndex

                if(HardkeyEnum.HKEY_STATE_ROTATE_LEFT === iKeyState){
                    if(-1 === prPreviousRREIndex){
                        prCurrentRREIndex = 2
                    }
                    else if(prPreviousRREIndex > 0){
                        prCurrentRREIndex--
                    }
                    else{
                        // Do nothing
