# Pro file for the Tata X445 HMI UnitTest
...
!include(gtest_dependency/gtest_dependency.pri){
    error("Could not include gtest_dependency");
}
...
linux-oe-g++ {
    ...
    DEFINES += USE_WAYLAND_IVI USE_RESOURCE_LAYER
    PLATFORM = linux
    CONFIG += c++11 qtquickcompiler wayland-compositor wayland-egl
    COMMONAPI_INCLUDE=$$(PKG_CONFIG_SYSROOT_DIR)/usr/include/CommonAPI-3.1/
    GTEST_INCLUDE=$$(PKG_CONFIG_SYSROOT_DIR)/usr/include/gtest/
    GMOCK_INCLUDE=$$(PKG_CONFIG_SYSROOT_DIR)/usr/include/gmock/
    CONFIG += resources_big

	QMAKE_INCDIR += $$GTEST_INCLUDE
	QMAKE_INCDIR += $$GMOCK_INCLUDE
